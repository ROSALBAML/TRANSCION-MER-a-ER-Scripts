...
DROP SCHEMA IF EXISTS `LibroAutorEdición` ;
...
CREATE TABLE IF NOT EXISTS `LibroAutorEdición`.`AUTOR` (
  ...
  CONSTRAINT `fk_AUTOR_LIBRO1`
    ...
    REFERENCES `LibroAutorEdición`.`LIBRO` (`ISBN`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;

CREATE INDEX `fk_AUTOR_LIBRO1_idx` ON `LibroAutorEdición`.`AUTOR` (`LIBRO_ISBN` ASC) VISIBLE;
...
CREATE INDEX `fk_EDICION_LIBRO_idx` ON `LibroAutorEdición`.`EDICION` (`LIBRO_ISBN` ASC) VISIBLE;
